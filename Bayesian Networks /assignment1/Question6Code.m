...
%calculate the CPTs for all the random variables in the given bayesian network. 
...
for i = 1:sz
   ...
   p4 = fullPECGHD(rw,3);
   rw = find(fullPHDAHR(:,1) == hd & fullPHDAHR(:,2)==a & fullPHDAHR(:,3)== hr);
   p5 = fullPHDAHR(rw,4);
...
for g = 1:2
        pg = find(fullPG(:,1) == g);
        bpg = find(fullPBPG(:,1) == 1 & fullPBPG(:,2) ==g);
        chga = find(fullPAGCH(:,3) == 2 & fullPAGCH(:,2) ==g & fullPAGCH(:,1)==2); 
        hdbpch = find(fullPCHBPHD(:,3) == 1 & fullPCHBPHD(:,2) ==1 & fullPCHBPHD(:,1)==2); 

        num = num + (fullPG(pg,2)* fullPBPG(bpg,3)*fullPAGCH(chga,4)* fullPCHBPHD(hdbpch,4));
...
for g = 1:2
    for bp =  1:2
        ...
        pg = find(fullPG(:,1) == g);
        bpg = find(fullPBPG(:,1) == bp & fullPBPG(:,2) ==g);
        chga = find(fullPAGCH(:,3) == 2 & fullPAGCH(:,2) ==g & fullPAGCH(:,1)==2); 
        hdbpch = find(fullPCHBPHD(:,3) == 1 & fullPCHBPHD(:,2) ==bp & fullPCHBPHD(:,1)==2); 
        
        din = din + (fullPG(pg,2)* fullPBPG(bpg,3)*fullPAGCH(chga,4)* fullPCHBPHD(hdbpch,4));
    end 
end
