...
disp('Plotting...');
plot(1:50,collectPixelOnWarm, 'r', 1:50, collectPixelOn, 'g', 'LineWidth',1);
axis([1 T 0 2500]);
xlabel('iteration t');
ylabel('#pixels ON');
legend('Initialize to noisy image','Initialized to zero');
title('WP = 1, WL = 0.8 when ignoring first 15 samples');
%title('WP = 1, WL = 3 when ignoring first 15 samples'); 
